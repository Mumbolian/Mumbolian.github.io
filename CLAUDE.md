...
T-Man's Snooze Schedule is a pure client-side nap scheduling app for parents. The entire application is contained in a single `index.html` file with embedded CSS and JavaScript—no build system, no dependencies, no backend.
...
Separate from constraint satisfaction—runs automatically even without constraints:
...
- Example: [30, 60, 30] (120 min total) → [45, 90, 45] (180 min target)
...
2. Test mode switching (2 ↔ 3 naps)
3. Test constraints:
...
- **Amber haze**: Schedule cards get amber border + "⚠️ Adjusted +X min" when nap lengths modified
- **Configure button highlight**: Gets amber glow when adjustments active
- **Constraint indicators**: Button shows count: "🎯 Constraints (2)"
- **Success/warning messages**: Color-coded in `warningBox` div (green = success, red = failure)
- **Wake window display**: Each schedule item shows "↑ Xh Ymin awake" duration
- **Preference mismatch**: Toggle switch indicates when actual mode differs from preference

### Time Handling

- `toMinutes(timeStr)`: Converts "HH:MM" → minutes since midnight (line ~1249)
- `fromMinutes(totalMinutes)`: Converts minutes → "HH:MM" with next-day handling (line ~1256)
